...
class Discriminator(nn.Module):
    def __init__(self, num_channels, base_filter, image_size):
        ...
        self.input_conv = ConvBlock(
            num_channels, base_filter, 3, 1, 1, activation="lrelu", norm=None
        )

        self.conv_blocks = nn.Sequential(
            ConvBlock(
                base_filter, base_filter, 3, 2, 1, activation="lrelu", norm="batch"
            ),
            ConvBlock(
                base_filter, base_filter * 2, 3, 1, 1, activation="lrelu", norm="batch"
            ),
            ConvBlock(
                base_filter * 2,
                base_filter * 2,
        ...
        self.dense_layers = nn.Sequential(
            DenseBlock(
                base_filter * 8 * image_size // 16 * image_size // 16,
                base_filter * 16,
                activation="lrelu",
                norm=None,
            ),

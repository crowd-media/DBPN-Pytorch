...
parser.add_argument(
    "--pretrained_iter", type=int, default=100, help="number of epochs to train for"
)
parser.add_argument("--pretrained", type=bool, default=False)
parser.add_argument(
    "--nEpochs", type=int, default=2000, help="number of epochs to train for"
)
parser.add_argument("--snapshots", type=int, default=25, help="Snapshots")
parser.add_argument("--start_iter", type=int, default=1, help="Starting Epoch")
parser.add_argument(
    "--lr", type=float, default=1e-4, help="Learning Rate. Default=0.01"
)
...
parser.add_argument(
    "--patch_size", type=int, default=60, help="Size of cropped HR image"
)
parser.add_argument(
    "--pretrained_sr",
    default="dl00DBPNLLPIRM_pretrained_50.pth",
    help="sr pretrained base model",
)
parser.add_argument("--load_pretrained", type=bool, default=False)
parser.add_argument(
    "--pretrained_D",
    default="dnnDBPNLLPIRM_RESNET_epoch_Discriminator_499.pth",
    help="sr pretrained base model",
)
parser.add_argument("--load_pretrained_D", type=bool, default=False)
parser.add_argument(
    "--feature_extractor", default="VGG", help="Location to save checkpoint models"
)
...
parser.add_argument(
    "--prefix", default="PIRM_VGG", help="Location to save checkpoint models"
)
...
def train_pretrained(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        print(
            "Epoch: [%2d] [%4d/%4d] G_loss_pretrain: %.8f"
...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        vgg_loss = opt.w2 * sum(
            [
                MSE_loss(fake_feature[i], real_feature[i].detach())
        ...
        style_loss = opt.w4 * sum(
            [
                MSE_loss(
                    utils.gram_matrix(fake_feature[i]),
                    utils.gram_matrix(real_feature[i]).detach(),
                )
                for i in range(len(real_feature))
            ]
        )
        ...
        print(
            "Epoch: [%2d] [%4d/%4d] G_loss: %.8f, D_loss: %.8f, mse:%.8f, perceptual: %.8f, style: %.8f, adv: %.8f"
            % (
                (epoch),
                (iteration),
    ...
    print(
        "===> Epoch {} Complete: Avg. Loss G: {:.4f} D: {:.4f} MSE: {:.4f} Perceptual: {:.4f} Style: {:.4f} Adv: {:.4f}".format(
            epoch,
            G_epoch_loss / len(training_data_loader),
            D_epoch_loss / len(training_data_loader),
            mse_epoch_loss / len(training_data_loader),
            feat_epoch_loss / len(training_data_loader),
...
def checkpoint(epoch, pretrained_flag=False):
    if pretrained_flag:
        ...
        model_out_path_D = (
            ...
            + opt.prefix
            + opt.feature_extractor
            + "_epoch_Discriminator_{}.pth".format(epoch)
        )
...
###Discriminator
D = Discriminator(
    num_channels=3, base_filter=64, image_size=opt.patch_size * opt.upscale_factor
)
